 ...
 */
utils.versionCompare = function(left, right) {
		...
		console: loggerFactory("console", {
			...
			format: function(log){
				...
				var now = this.dateFormat(new Date()),
					...
					message = log.message || "";
/*				
				//Embedded RTC 성능 개선 지원 관련 TIMEDIFF 로그 추가(2016.11.02)
...
var Call = utils.Extend(utils.Event, {
	...
	onOtherConnect: function(pid, uid, sender){
		...
		if(!this.peers[pid]){
			...
			// MCU DataChannel 대응 Header(Master Block) 정보 수정 (jun, 2016.11.17)
		...
		switch(command){
			...
			case "ON_CONNECT":
				...
				if(body.data.others.length > 0){
					// MCU DataChannel 대응 Header(Master Block) 정보 수정 (jun, 2016.11.17) : sender 파라미터 추가(body.data.others[0])
			...
			case "ON_CLOSE":
				...
				// [SDK v2.2.19] : Network 전환시 reconnect(jun, 2017.02.22)
				// Network 전환으로 인한 P2P disconnect일 경우 처리 이전 토큰 채널 퇴장 reason으로 reconnect, timeout case 발생
...
var Peer = utils.Extend(utils.Event, {
	initialize: function(call, id, uid, localStream, config){
		...
		// MCU DataChannel 대응 Header(Master Block) 정보 수정 (jun, 2016.11.17) : other peer정보(sender) 추가
	...
	setEvent: function(){
		...
		pc.oniceconnectionstatechange = utils.bind(function(e){
			...
			if(connectionState === "FAILED"){
				// [SDK v2.2.18] : p2p failed 통계 오류 수정 - 최초 커넥션 실패일 경우에만 서버로 전송(jun, 2016.12.15)
				if(!this.connected){
					Logger.error("cdmn", {
						klass: "Peer",
						method: "setEvent",
	...
	replaceBandWidth: function(sdp){
		...
		/*
		 * [SDK v2.2.16] : 이종 브라우저간(예 : 파이어폭스-크롬) bandwidth sdp 처리 방법 변경
		 * chrome : audio video data
		 * firefox : sdparta_0 sdparta_1 sdparta_2
		*/
		for(var i=0; i<bundles.length; i++){
			if(bundles[i] === "audio" || bundles[i] === "sdparta_0"){
				// [SDK v2.2.18] : audio codec이 opus가 아닌 경우 bandwidth 변경시 오류 발생되므로 예외처리(jun, 2016.11.24)
	...
	return utils.Extend(utils.Event, {
		...
		setEvent: function(){
			...
			function onmessage(data){
				...
				* 적용 SDK web(windows, mac, linux) : 2.2.17 이상, ios : 2.2.8 이상, android : 2.2.9 이상*/
				var versionCheck = utils.versionCompare(this.peer.peers.sdkversion, PLATFORM_VERSION[this.peer.peers.platformtype]);	//lower version : -1, exact version : 0, higher version : 1
				
				if(versionCheck === -1) {
					var type = dv.getInt32(20);
				} else {
					var type = dv.getInt32(54);
				}
		...
		sendText: function(text, success, error){
			var dc = this.dataChannel,
				id = getUniqId(),
				tokenId = this.peer.call.getToken(),		// Header(Master Block) 정보 수정 (jun, 2016.11.17) 
			...
			* 적용 SDK web(windows, mac, linux) : 2.2.17 이상, ios : 2.2.8 이상, android : 2.2.9 이상*/
			...
			dv.setFloat64(pos, id);				// 데이터 아이디( 8 byte) : long 전송 데이터 스트림의 고유 아이디
			pos += 8;
			dv.setInt32(pos, 0);					// 헤더 타입(4 byte) : int, Master/Frag Block Header Type, 0: Master
			pos += 4;
			if(versionCheck !== -1) {
				// Sender 아이디(34 byte) : 데이터 전송 사용자 아이디 17자(Token ID)
				var i = pos, j = 0;
				pos += 34;
				for (; i<pos; i=i+1) {
			...
			dv.setFloat64(pos, totalSize);		// 데이터 크기( 8 byte) : long, Application 데이터 전체 크기
			pos += 8;
			dv.setInt32(pos, 0);					// 데이터 유형(4 byte) : int, 0 : 텍스트, 1 : 파일
			pos += 4;
			dv.setInt32(pos, fragCount);		// Block Count(4 byte) : int, 전체 데이터 Block 수
			pos += 4;
			dv.setInt32(pos, 0);					// Block Index(4 byte) : 전송하는 Block의 index
			pos += 4;
			dv.setInt32(pos, arr[0].byteLength);	// Block 데이터 크기(4 byte) : int, 전송하는  Application Data의 크기
		...
		sendFile: function(file, success, error){
			...
			* 적용 SDK web(windows, mac, linux) : 2.2.17 이상, ios : 2.2.8 이상, android : 2.2.9 이상*/
			...
			mdv.setFloat64(pos, id);				// 데이터 아이디( 8 byte) : long 전송 데이터 스트림의 고유 아이디
			pos += 8;
			mdv.setInt32(pos, 0);					// 헤더 타입(4 byte) : int, Master/Frag Block Header Type, 0: Master
			pos += 4;
			if(versionCheck !== -1) {
				// Sender 아이디(34 byte) : 데이터 전송 사용자 아이디 17자(Token ID)
				var i = pos, j = 0;
			...
			mdv.setFloat64(pos, totalSize);		// 데이터 크기( 8 byte) : long, Application 데이터 전체 크기
			pos += 8;
			mdv.setInt32(pos, 1);					// 데이터 유형(4 byte) : int, 0 : 텍스트, 1 : 파일
			pos += 4;
			...
			mdv.setInt32(pos, fragCount);		// Block Count(4 byte) : int, 전체 데이터 Block 수
			pos += 4;
			mdv.setInt32(pos, index);				// Block Index(4 byte) : 전송하는 Block의 index
			pos += 4;
			// Block 데이터 크기(4 byte) : int, 전송하는  Application Data의 크기
